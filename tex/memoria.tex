...
Una vez obtenidos los tiempos al inicio y finalización de la ejecución, se realiza el cálculo conveniente para hallar, mediante las estructuras \texttt{timespec}, los segundos y nanosegundos transcurridos. Se han modificado los programas para que muestren en la salida únicamente este valor numérico, de forma que se puedan recoger todos los resultados en un único archivo de datos que sirva de entrada para \texttt{gnuplot}.
...
Además, se ha añadido a este guion la funcionalidad necesaria para comunicarse con \texttt{gnuplot} y crear una gráfica a partir del archivo de datos generado por uno de los programas, y crear también la gráfica de la función de ajuste. En cada caso, el Makefile (sección \ref{makefile}) se encarga de proporcionar los parámetros necesarios al guion y de redirigir la salida al archivo correcto, de forma que el usuario solo debe ejecutar órdenes sencillas para generar gráficas de datos y ajustes de todos los programas.
...
\subsection{Algoritmos teóricamente $\mathcal{O}(n^2)$}
  ...
  El mejor ajuste práctico ha resultado ser: $f(n)=-6.82525\cdot 10^{-12} \cdot n^3+1.26402\cdot 10^{7}\cdot n^2-0.000661563 \cdot n+0.999054$, con
  una media de residuos cuadráticos de $0.0037376$\\
...
\begin{itemize}
  ...
  El ajuste que deberíamos haber obtenido: $f(n)=a \cdot n^2+b \cdot n+c$\\
    ...
    Ajuste & Media de los residuos cuadráticos\\
    \hline
    $f(n)=1.69704\cdot 10^{-5}\cdot n -0.0271906$ & $0.012942$ \\
    \hline
    $f(n)= 1.61933\cdot 10^{-9}\cdot n^2+9.06663\cdot 10^{-7}\cdot n+-0.000845545$ & $0.00437183$ \\
    \hline
    $f(n)= 1.52761\cdot 10^{-5}\cdot n \cdot log(n)-0.000128103\cdot n0.00641101 \cdot log(n)-0.0197314$ & $0.00463731$ \\
    \hline
    \end{tabular}
  \end{itemize}
...
\begin{itemize}
  ...
  \item Ordenación heapsort\\
  El mejor ajuste ha resultado ser: $f(n)=2.05288 \cdot 10^{-8}\cdot nlog(n) + 4.34162\cdot 10^{-9}\cdot n+a2 \cdot 1.04338 
  \cdot 10^{-6}log(n)-3.42946 \cdot 10^{-6}$, con una media de residuos cuadráticos de $7.02599 \cdot 10^{-6}$\\
